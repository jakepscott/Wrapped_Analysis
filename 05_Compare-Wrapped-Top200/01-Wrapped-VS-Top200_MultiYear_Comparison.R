  ...
  Feats_Data %>% 
    ...
    labs(title="How <span style='color: #1DB954'>**your**</span> music compares to <span style='color: #A9A9A9'>**popular**</span> music on Spotify?") +
  ...
  (p2 <- Full_Wrapped %>% select(Playlist,Artist) %>% distinct(Playlist,Artist) %>% 
      group_by(Playlist) %>% 
