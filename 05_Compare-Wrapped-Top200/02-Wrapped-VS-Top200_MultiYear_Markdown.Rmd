...
Feats_Data <- Full_Comparison %>% 
  ...
  Feats_Data %>% 
    ...
    facet_wrap(~Feature,scales = "free_y",labeller = label_wrap_gen(width = 20)) +
...
genres <- Full_Comparison %>% select(Wrapped, Playlist,`Percent Hip Hop`:`Percent Other`)
  ...
  genres %>% pivot_longer(cols=`Percent Hip Hop`:`Percent Other`,names_to="Genre",values_to="Percent") %>% 
    ...
    labs(title="<span style='color: #1DB954'>**Your**</span> favorite genres versus which ones were <span style='color: #A9A9A9'>**popular**</span>",
         subtitle = "Percent of songs in given genre") +
    theme(plot.title = element_markdown(size = rel(1.8)),
          axis.title = element_blank(),
          plot.title.position = "plot",
          strip.text = element_text(size=rel(.9)),
