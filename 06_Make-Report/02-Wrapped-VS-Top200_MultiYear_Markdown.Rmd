...
knitr::opts_chunk$set(echo = F, warning = F, message = F, fig.align = 'center',fig.height = 8, fig.showtext=T)
...
# Make Data-------------------------------------------------------
load(here("06_Make-Report/data/keys"))
source(here("06_Make-Report/functions/01-Obtain_Tracks_Function.R"))
source(here("06_Make-Report/functions/02-Obtain_Features_Function.R"))
source(here("06_Make-Report/functions/03-Obtain_Lyrics_Function.R"))
source(here("06_Make-Report/functions/04-Analyze_Lyrics_Function.R"))
source(here("06_Make-Report/functions/05-Compare_Playlists_Function.R"))


# Getting User Wrapped Playlists ------------------------------------------
user_playlists <- getPlaylists(params$uri,token = keys) %>% as_tibble() %>% filter(tracks>0 & str_detect(name,"Your Top Songs")==T)
...
#Grouped 
  Grouped_Wrapped <- Wrapped_Playlist_Data %>% 
...
#Grouped
  Grouped_Top_200 <- read_rds(here("data/Top200_Weighted_Playlist_Data.rds")) %>% 
    rename("Median Loudness (dB)"=`Median Loudness`,
           "Median Tempo (BPM)"=`Median Tempo`,
           "Percent of Songs That Are Explicit"=`Percent Explicit`) %>% 
    mutate(Wrapped="No",
...
# Joining Wrapped and Top 200 ---------------------------------------------
  ...
  Full_Grouped <- Grouped_Wrapped %>% bind_rows(Grouped_Top_200)
...
```{r Add Percent Explicit Words to Grouped data}
...
#Add to Grouped data
Full_Grouped <- Full_Grouped %>% left_join(Per_Explicit_Words_Combined)

```


```{r Music Features, fig.height=7.5}
Feats_Data <- Full_Grouped %>% 
  ...
  Feats_Data %>% 
    ...
    facet_wrap(~Feature,scales = "free_y",labeller = label_wrap_gen(width = 13)) +
    labs(title="How <span style='color: #1DB954'>**your**</span> music compares to <span style='color: #A9A9A9'>**popular**</span> music",
         subtitle = "Measures like \"valence\" generated by Spotify and used in their internal algorithms",
...
```{r Genres}
genres <- Full_Grouped %>% select(Wrapped, Playlist,`Percent Hip Hop`:`Percent Other`) 
names(genres) <- names(genres) %>% str_remove("Percent ")
  ...
  genres %>% pivot_longer(cols=`Hip Hop`:`Other`,names_to="Genre",values_to="Percent") %>% 
    ...
    geom_point(aes(color=Wrapped),size=5) +
    scale_color_manual(values=c("#A9A9A9","#1DB954")) +
    scale_y_continuous(expand = expansion(mult =  0.3), n.breaks = 4,labels=function(x) paste0(x,"%")) +
    scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
...
```{r Sentiment Categories}
  ...
  sentiments <- Full_Grouped %>% select(Wrapped,Playlist,`Percent of Words in Trust Category`:`Percent of Words in Anticipation Category`)
  ...
  sentiments %>% 
    ...
    geom_point(aes(color=Wrapped),size=5) +
    scale_color_manual(values=c("#A9A9A9","#1DB954")) +
    scale_y_continuous(expand = expansion(mult =  0.2), n.breaks = 4,labels=function(x) paste0(x,"%")) +
    scale_x_discrete(guide = guide_axis(n.dodge = 2)) + 
    facet_wrap(~Genre,scales = "free_y") +
    labs(title="What were the vibes of <span style='color: #1DB954'>**your**</span> music versus <br/><span style='color: #A9A9A9'>**popular**</span> music",
...
# Overall Sentiment -------------------------------------------------
  Full_Grouped %>% 
    ...
    scale_x_discrete(guide = guide_axis(n.dodge = 2)) +
    guides(color="none") +
    labs(title="What was the overall sentiment of <span style='color: #1DB954'>**your**</span> music  <br> versus <span style='color: #A9A9A9'>**popular**</span> music",
...
```{r Top Clean Words}
  ...
  top_5_words <- interesting_words %>% 
    ...
    mutate(total=sum(n),
           percent=(n/total)) %>% 
    slice_max(percent, n = 10, with_ties = FALSE) %>%
...
interesting_words_explicit <- words %>% 
  ...
  ggplot(top_5_words_explicit, aes(words_clean, percent, fill = Playlist)) +
    ...
    scale_x_reordered() +
    scale_fill_manual(values = c("#5BC680","#1DB954","#16873D","#1B3B26")) +
    labs(title="Top Words by Year, Including Explicit Words",
         subtitle = "Not including so-called stop words such as \"the\" and \"for\"",
         y="Percent of all non-stop words",
         caption = "Plot: @jakepscott2020 | Data: Spotify and Genius") +
    theme(plot.title = element_text(size = rel(2)),
...
# Most Unique Words -------------------------------------------------------
  ...
  Top_10_Relative_Importance %>%
    ...
    scale_x_reordered() +
    scale_fill_manual(values = c("#5BC680","#1DB954","#16873D","#1B3B26")) +
    labs(title="Most Uniquely Important Words by Year",
